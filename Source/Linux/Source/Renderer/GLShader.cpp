...
namespace ZED
{
	namespace Renderer
	{
			...
			const ZED_MEMSIZE p_Count )
		{
			...
			{
				ZED_SHADER_CONSTANT_MAP	ConstantMap;
				ConstantMap.Location = zglGetUniformLocation(
					m_ProgramID, p_pConstants[ i ].pName );
				ConstantMap.Index = p_pConstants[ i ].Index;
				ConstantMap.Type = p_pConstants[ i ].Type;
#ifdef ZED_BUILD_DEBUG
				ConstantMap.pName = p_pConstants[ i ].pName;
#endif
				m_Constants.push_back( ConstantMap );
			}	
		...
		ZED_UINT32 GLShader::SetConstantData( const ZED_UINT32 p_Index,
			const void *p_pValue )
		{
			if( p_Index > m_Constants.size( ) )
			{
				zedTrace( "[ZED::Renderer::GLShader::SetConstantData] <ERROR> "
					"Constant index outside of range [Maximum is %d, "
					"requested %d]\n", m_Constants.size( ), p_Index );

				return ZED_FAIL;
			}

			switch( m_Constants[ p_Index ].Type )
			{
				case ZED_FLOAT1:
				{
					zglUniform1f( m_Constants[ p_Index ].Location,
				...
				case ZED_FLOAT2:
				{
					zglUniform2fv( m_Constants[ p_Index ].Location, 1,
						static_cast< const GLfloat * >( p_pValue ) );
					break;
				}
				case ZED_FLOAT3:
				{
					zglUniform3fv( m_Constants[ p_Index ].Location, 1,
						static_cast< const GLfloat * >( p_pValue ) );
					break;
				}
				case ZED_FLOAT4:
				{
					zglUniform4fv( m_Constants[ p_Index ].Location, 1,
						static_cast< const GLfloat * >( p_pValue ) );
					break;
				}
				case ZED_INT1:
				{
					zglUniform1i( m_Constants[ p_Index ].Location,
						*( reinterpret_cast< const GLint * >( &p_pValue ) ) );
					break;
				}
				case ZED_INT2:
				{
					zglUniform2iv( m_Constants[ p_Index ].Location, 1,
						static_cast< const GLint * >( p_pValue ) );
					break;
				}
				case ZED_INT3:
				{
					zglUniform3iv( m_Constants[ p_Index ].Location, 1,
						static_cast< const GLint * >( p_pValue ) );
					break;
				}
				case ZED_INT4:
				{
					zglUniform4iv( m_Constants [ p_Index ].Location, 1,
						static_cast< const GLint * >( p_pValue ) );
					break;
				}
				case ZED_MAT2X2:
				{
					zglUniformMatrix2fv( m_Constants[ p_Index ].Location, 1,
						GL_FALSE,
						static_cast< const GLfloat * > ( p_pValue ) );
					break;
				}
				case ZED_MAT3X3:
				{
					zglUniformMatrix3fv( m_Constants[ p_Index ].Location,
						1, GL_FALSE,
						static_cast< const GLfloat * >( p_pValue ) );
					break;
				}
				case ZED_MAT4X4:
				{
					zglUniformMatrix4fv( m_Constants[ p_Index ].Location, 1,
			...
			const ZED_CHAR8 *p_pConstantName )
		{
			...
			{
				zedTrace( "[ZED::Renderer::GLShader::GetConstantIndex] <WARN> "
					"Could not find location from name\n" );
				...
				switch( Location )
				{
					...
					{
						zedTrace( "[ZED::Renderer::GLShader::GetConstantIndex]"
					...
					{
						zedTrace( "[ZED::Renderer::GLShader::GetConstantIndex]"
							" <ERROR> Program object not valid\n" );

						break;
					}
					default:
					{
						m_NameLocationMap.insert( std::pair< std::string,
							GLuint >( p_pConstantName, Location ) );

						p_Location = Location;

						// Get the type
						GLint GLType;
						zglGetActiveUniformsiv( m_ProgramID, 1, &Location,
							GL_UNIFORM_TYPE, &GLType );
