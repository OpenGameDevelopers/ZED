...
namespace ZED
{
	namespace Arithmetic
	{
		...
		{
			// SIMD would be a real help in here
		...
		ZED_FLOAT32 Vector2::Distance( const Vector2 &p_Other ) const
		{
			...
			return ZED::Arithmetic::SquareRoot( X*X + Y*Y );
		}
		...
		{
			p_Out << "Vector2 [ " << p_Source[ 0 ] << ", " << p_Source[ 1 ]
				<< " ]" << std::endl;
			return p_Out;
		}
		...
		{
			return Vector2( p_Scalar*p_Self[ 0 ], p_Scalar*p_Self[ 1 ] );
		}
